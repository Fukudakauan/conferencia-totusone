...
SSL_FOLDER = os.path.join(DATA_DIR, 'ssl')
...
def criar_banco():
    ...
    try:
        ...
        c.execute("""
            CREATE INDEX IF NOT EXISTS idx_produtos_loja_id
            ON produtos(loja_id);
        """)
...
def importar_arquivo(caminho_arquivo, loja_id):
    ...
    try:
        ...
        try:
            ...
            if valores:
                execute_batch(c, sql_insert, valores, page_size=500)
...
def upload():
    ...
    try:
        ...
        resultado = importar_arquivo(caminho_temp, session['usuario_id'])

        if isinstance(resultado, str) and not resultado.startswith("[OK]"):
...
def consultar_produto():
    ...
    entrada_raw = str(data.get('codigo') or data.get('entrada') or data.get('produto') or '').strip()

    if not entrada_raw:
        return jsonify({'status': 'erro', 'mensagem': 'Informe um código ou referência.'})
...
def gerar_relatorio():
    ...
    nome_arquivo = f"relatorio_{tipo}_loja_{loja_id}.pdf"
    return send_file(
        buffer,
        as_attachment=True,
